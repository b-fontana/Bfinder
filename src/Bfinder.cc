...
#include <iostream>
//Bfinder
#include "UserCode/Bfinder/interface/format.h"
//#include "UserCode/Bfinder/interface/TriggerBooking.h"
  ...
  try{
	...
	if (input_tracks.size() == 0)
	...
	else
	  {
		...
		//MuonInfo section{{{
		int mu_hindex = -1;
		for(std::vector<pat::Muon>::const_iterator mu_it=input_muons.begin(); mu_it != input_muons.end() ; mu_it++)
		  {
			...
			if (!muon::isGoodMuon(*mu_it,muon::TMOneStationTight)) ;
			else
			  {
			    ...
			    if(!mu_it->innerTrack().isNonnull()) ;
			      ...
			      {
				MuonCutLevel->Fill(3);
				if (fabs(mu_it->innerTrack()->dxy(thePrimaryV.position())) >= 3. || fabs(mu_it->innerTrack()->dz(thePrimaryV.position()))  >= 30.) ;
				else
				  {
				    MuonCutLevel->Fill(4);
				    if (mu_it->innerTrack()->hitPattern().pixelLayersWithMeasurement()<1 || mu_it->innerTrack()->normalizedChi2()>1.8) ;
				    else
				      {
					MuonCutLevel->Fill(5);
			...
			for(int tq = 0; tq < reco::TrackBase::qualitySize; tq++)
			  {
			    if (mu_it->innerTrack()->quality(static_cast<reco::TrackBase::TrackQuality>(tq)))
			      MuonInfo.innerTrackQuality[MuonInfo.size] += 1 << (tq);
		    ...
		    else if(doMuPreCut_ && (!muon::isGoodMuon(*mu_it,muon::TMOneStationTight)) )
		      {
			MuonInfo.isNeededMuon[MuonInfo.size] = false;
		      }
		    else
		...
		int PassedTrk = 0;
		for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin(); tk_it != input_tracks.end(); tk_it++)
		  {
		...
		int mu1_hindex = -1;
		bool gogogo = false;
		for(std::vector<pat::Muon>::const_iterator mu_it1=input_muons.begin(); mu_it1 != input_muons.end(); mu_it1++)
		  {
		    ...
		    int mu2_index = -1;
		    int mu2_hindex = -1; 
		    for(std::vector<pat::Muon>::const_iterator mu_it2=input_muons.begin(); mu_it2 != input_muons.end(); mu_it2++)
		      {
			//check if muon track is non null
			...
			if(Bchannel_[0] == 1)
			  BranchOut2MuTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, KAON_MASS, 1);
			...
			if(Bchannel_[1] == 1)
			  BranchOut2MuTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, PION_MASS, 2);
			...
			if(Bchannel_[2] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, KSHORT_MASS, 0.3, PION_MASS, PION_MASS, 3, 1);
			...
			if(Bchannel_[3] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, KSTAR_MASS, 0.4, KAON_MASS, PION_MASS, 4, 0);
			  			
			//////////////////////////////////////////////////////////////////////////
			// RECONSTRUCTION: J/psi + K* (Pi-, K+)
			//////////////////////////////////////////////////////////////////////////
			mass_window[0] = 4.3;
			mass_window[1] = 6.4;

			if(Bchannel_[4] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, KSTAR_MASS, 0.4, PION_MASS, KAON_MASS, 5, 0);
			...
			if(Bchannel_[5] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, PHI_MASS, 0.1, KAON_MASS, KAON_MASS, 6, 0);
			...
			if(Bchannel_[6] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, -1, 1.6, PION_MASS, PION_MASS, 7, 0);
			...
			if(Bchannel_[7] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, LAMBDA_MASS, 0.3, PROTON_MASS, PION_MASS, 8, 1);
			...
			if(Bchannel_[8] == 1)
			  BranchOut2MuX_XtoTkTk(BInfo, input_tracks, isNeededTrack, v4_mu1, v4_mu2, muonPTT, muonMTT, B_counter, mass_window, JPSI_MASS, LAMBDA_MASS, 0.3, PION_MASS, PROTON_MASS, 9, 1);
		...
		for(std::vector<pat::GenericParticle>::const_iterator tk_it=input_tracks.begin(); tk_it != input_tracks.end() ; tk_it++)
		  {
			...
			for(int tq = -1; tq < reco::TrackBase::qualitySize; tq++)
			  {
                            if (tk_it->track()->quality(static_cast<reco::TrackBase::TrackQuality>(tq)))
			      TrackInfo.trackQuality[TrackInfo.size] += 1 << (tq);
			  }
		      }
		...
		for(std::vector<const reco::Candidate *>::iterator mCands = sel_cands.begin(); mCands != sel_cands.end(); mCands++)
		  {
			...
			if(nGenMo == 0)
			  GenInfo.mo1[geninfo_idx] = int(mCands-sel_cands.begin());
			if(nGenMo == 1)
			  GenInfo.mo2[geninfo_idx] = int(mCands-sel_cands.begin());
		      }
		  }
	      }
		...
		for(std::vector<const reco::Candidate *>::iterator mCands = sel_cands.begin(); mCands != sel_cands.end(); mCands++)
		  {
		    if((*sCands)->daughter(nGenDa) == *mCands)
		      {
			if(nGenDa == 0)
			  GenInfo.da1[geninfo_idx] = int(mCands-sel_cands.begin());
			if(nGenDa == 1)
			  GenInfo.da2[geninfo_idx] = int(mCands-sel_cands.begin());
		      }
		  }
	      }
	  }
...
bool Bfinder::GetAncestor(const reco::Candidate* p)
{
  if(p->numberOfMothers()==0)
  ...
  else
    {
      ...
      if(abs(int(mpid/100) % 100) == 5)
	return true;
      else
	return GetAncestor(MyMom);
    }
}

void Bfinder::BranchOut2MuTk(BInfoBranches &BInfo, std::vector<pat::GenericParticle> input_tracks, std::vector<bool> isNeededTrack, TLorentzVector v4_mu1, TLorentzVector v4_mu2,reco::TransientTrack muonPTT, reco::TransientTrack muonMTT, std::vector<int> &B_counter, float *mass_window, float MuMu_MASS, float Tk_MASS, int channel_number)
{
  if(channel_number > (int)Bchannel_.size())
    {
      printf("Exceeding defined # of channel, exit");
      return;
    }
  ...
  for(std::vector<pat::GenericParticle>::const_iterator tk_it1=input_tracks.begin(); tk_it1 != input_tracks.end() ; tk_it1++)
    {
...
void Bfinder::BranchOut2MuX_XtoTkTk( BInfoBranches &BInfo, std::vector<pat::GenericParticle> input_tracks, std::vector<bool> isNeededTrack, TLorentzVector v4_mu1, TLorentzVector v4_mu2, reco::TransientTrack muonPTT, reco::TransientTrack muonMTT, std::vector<int> &B_counter, float *mass_window, float MuMu_MASS, float TkTk_MASS, float TkTk_window, float Tk1_MASS, float Tk2_MASS, int channel_number, int fit_option)
{
  if(channel_number > (int)Bchannel_.size())
    {
      printf("Exceeding defined # of channel, exit");
  ...
  for(std::vector<pat::GenericParticle>::const_iterator tk_it1=input_tracks.begin(); tk_it1 != input_tracks.end() ; tk_it1++)
    {
      ...
      for(std::vector<pat::GenericParticle>::const_iterator tk_it2=input_tracks.begin(); tk_it2 != input_tracks.end() ; tk_it2++)
	{
	  ...
	  if(TkTk_MASS > 0)
	    {
	      if(fabs((v4_tk1+v4_tk2).Mag()-TkTk_MASS)>TkTk_window) continue;
	    }
	  else
	    {
	      if (fabs((v4_tk1+v4_tk2).Mag())>TkTk_window) continue;//if no tktk mass constrain, require it to be at least < some window
	    }
